...
export default function VideoView() {
  ...
  const handleUrlSubmit = (parseResult: {
    type: string;
    id: string;
    url: string;
  }) => {
      ...
      <div className="container py-6 px-4 sm:px-6 flex-grow">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <VideoPlayer videoId={videoId} onTimeUpdate={setCurrentTime} onReady={() => console.log("Video ready")} />
            ...
            {/* Create Note Form */}
            {isCreatingNote && <div className="p-4 border rounded-lg bg-card">
                <h3 className="text-lg font-medium mb-2">Create a Note</h3>
                <NoteEditor currentVideoTime={currentTime} onSave={handleSaveNote} onCancel={() => setIsCreatingNote(false)} />
              </div>}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
              ...
              <TabsContent value="notes" className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 pt-0 space-y-3">
                  {notes.length === 0 ? <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                    ...
                    </div> : notes.map(note => <NoteCard key={note.id} note={note} onDelete={() => handleDeleteNote(note.id)} onTimestampClick={note.videoTimestamp ? () => handleTimestampClick(note.videoTimestamp!.seconds) : undefined} />)}
                </div>
              </TabsContent>
              
              {/* Transcript Tab */}
              <TabsContent value="transcript" className="flex-1 flex flex-col h-full overflow-hidden">
                <TranscriptViewer videoId={videoId} transcript={transcript} onSegmentClick={handleTimestampClick} isLoading={isLoadingTranscript} />
              </TabsContent>
            </Tabs>
          </div>
        </div>
